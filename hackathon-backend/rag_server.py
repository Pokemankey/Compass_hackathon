...
llm_35_00 = ChatOpenAI(model_name="gpt-3.5-turbo-1106", temperature=0)
...
vectorstore = Qdrant(client = qclient, collection_name = "compass", embeddings = embeddings)
...
rag_chain = (
    ...
    | ChatPromptTemplate.from_template("""You are a digital assistant for the Compass gaming event. Answer questions asked by the user only. IF THERE IS AN IMAGE URL IN THE CONTEXT SEND IT LIKE THIS <<image url here>> ALWAYS STICK TO THIS FORMAT
                                       if there is a url to a page always send it like this [[url here]] ALWAYS FOLLOW THIS FORMAT.
...
async def stream_llm_response(query: Query):
    print(query)
    response = query_full(query.query)
    # print(response)
    # sentence = ' '.join(filter(None, response))
    retTime = time.time()
    return {"sentence": response, "retTime":retTime}
